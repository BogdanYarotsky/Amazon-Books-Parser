...
func getChromeParsedHTMLs(urls []string) ([]*BookHTML, error) {
	...
	fmt.Println("Created Exec Allocator")
	// start a tab
	tab1, cancel := chromedp.NewContext(browser, chromedp.WithLogf(log.Printf))
	defer cancel()
	if err := chromedp.Run(tab1); err != nil {
		return nil, err
	}
	fmt.Println("Running first tab")
	...
	for i := 0; i < len(urls)-1; i++ {
		...
		fmt.Println("Created and added new tab to pool")
		defer cancel()
	}
	...
	for i, url := range urls {
		fmt.Println("Starting work on new thread")
...
func chromeFetchAndParse(page string, tab context.Context, chParsedHTML chan *BookHTML, chIsFinished chan bool) {
	...
	fmt.Println("Succesfully got response from", source)
